...
class model_arch(nn.Module):
    ...
    def forward(self, data):
        ...
        query_dict:
            rescaled_image: 查询图像的全尺寸RGB图像，形状为 [BVq, 3, H, W]。
            rescaled_mask: 查询图像的全尺寸掩码，形状为 [BVq, 1, H, W]。
            dzi_image: 查询图像的裁剪图像，形状为 [BVq, 3, S, S]。
            dzi_mask: 查询图像的裁剪掩码，形状为 [BVq, 1, S, S]。
            allo_RT: 查询图像的仿射变换矩阵，形状为 [BVq, 3, 3]。

        qnnb_dict:
            zoom_image: 查询图像的最近邻图像的裁剪图像，形状为 [BVqVn, 3, S, S]。
            zoom_mask: 查询图像的最近邻图像的裁剪掩码，形状为 [BVqVn, 1, S, S]。
            allo_RT: 最近邻图像的仿射变换矩阵，形状为 [BVqVn, 3, 3]。

        refer_dict:
            zoom_image: 参考图像的裁剪图像，形状为 [BVr, 3, S, S]。
            zoom_mask: 参考图像的裁剪掩码，形状为 [BVr, 1, S, S]。
            allo_RT: 参考图像的仿射变换矩阵，形状为 [BVr, 3, 3]。

        rand_dict:
            zoom_image: 随机图像的裁剪图像，形状为 [BVd, 3, S, S]。
            zoom_mask: 随机图像的裁剪掩码，形状为 [BVd, 1, S, S]。
            allo_RT: 随机图像的仿射变换矩阵，形状为 [BVd, 3, 3]。


        模型的输出也是一个字典 data_dict，包含以下键值对：
        ref_pd_mask: 参考图像的预测掩码，形状为 [BVr, 1, S, S]。
        que_pd_mask: 查询图像的预测掩码，形状为 [BVq, 1, S, S]。
        qnnb_pd_mask: 最近邻图像的预测掩码，形状为 [BVqVn, 1, S, S]。
        rand_pd_mask: 随机图像的预测掩码，形状为 [BVd, 1, S, S]。
        que_full_pd_mask: 查询图像全尺寸的预测掩码，形状为 [BVq, 1, H, W]。
        rm_loss: 参考图像的掩码损失。
        cm_loss: 裁剪图像的掩码损失。
        qm_loss: 查询图像全尺寸的掩码损失。
        Remb_loss: 旋转嵌入损失。
        Remb_logit: 旋转嵌入的logits，形状为 [BVq, VqVn + Vd + Vr]。
        delta_Rdeg: 旋转角度的差异，形状为 [BVq, VqVn + Vd + Vr]。
        '''
        dim_B = self.batch_size
        # 后面用dinov2提取特征
