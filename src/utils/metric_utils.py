...
def compute_query_pose_errors(
    data, gaussion_model,configs, training=False
):
    """
    Update:
        data(dict):{
    ...
    for bs in range(query_K.shape[0]):
        ...
        # query_pose_pred_homo是4x4的位姿矩阵
        # query_pose_pred事3X4的，没带最后一行
        
        pose_pred.append(query_pose_pred_homo)
        gs_refine = GS_refine()

        R_init = query_pose_pred_homo[:3,:3]
        T_init = query_pose_pred_homo[:3,-1]
        imag_h = img_orig_size[bs].tolist()[0]
        imag_w =    img_orig_size[bs].tolist()[1]
                # 这里是
        fx = query_K[bs][0,0]
        fy = query_K[bs][1,1]
        cx = query_K[bs][0,2]
        ...
        fovx = 2 * np.arctan(imag_w / (2 * fx))
        fovy = 2 * np.arctan(imag_h / (2 * fy))
        image_path = data['query_image_path']

        init_camera = Camera(colmap_id=bs,R=R_init.T,T=T_init,FoVx=fovx,FoVy=fovy,
                             Cx=cx,Cy=cy,image_width=imag_w,image_height=imag_h,uid=bs,
                             image_path=image_path,image_name = 'None')
        

        imag0 = cv2.imread(image_path)
        # 换一下颜色
        ...
        result = gs_refine.GS_Refiner(image = imag0_rgb_torch,init_camera=init_camera,gaussians=gaussion_model,
                                      mask = None)
